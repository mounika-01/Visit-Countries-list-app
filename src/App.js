...
class App extends Component {
  state = {countries: initialCountriesList}

  onToggleVisit = id => {
    this.setState(prevState => ({
      countries: prevState.countries.map(eachCountry =>
        eachCountry.id === id
          ? {...eachCountry, isVisited: !eachCountry.isVisited}
          : eachCountry,
      ),
    }))
  }
  ...
  render() {
    ...
    const visitedCountriesList = countries.filter(
        ...
        <ul className="details-container">
          {countries.map(eachCountry => (
            <CountryDetail
              ...
              onToggleVisit={this.onToggleVisit}
            />
          ))}
        </ul>
